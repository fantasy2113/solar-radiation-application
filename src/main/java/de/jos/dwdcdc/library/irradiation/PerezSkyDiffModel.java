...
final class PerezSkyDiffModel {
  ...
  private static Map<String, double[]> getFTab() {
    return Map.of(F_11, new double[]{-0.008, 0.130, 0.330, 0.568, 0.873, 1.132, 1.060, 0.678}, F_12,
        new double[]{0.588, 0.683, 0.487, 0.187, -0.392, -1.237, -1.600, -0.327}, F_13,
        new double[]{-0.062, -0.151, -0.221, -0.295, -0.362, -0.412, -0.359, -0.250}, F_21,
        new double[]{-0.060, -0.019, 0.055, 0.109, 0.226, 0.288, 0.264, 0.156}, F_22,
        new double[]{0.072, 0.066, -0.064, -0.152, -0.462, -0.823, -1.127, -1.377}, F_23,
        new double[]{-0.022, -0.029, -0.026, -0.014, 0.001, 0.056, 0.131, 0.251});
  }
  ...
  private void computeAirMass() {
    airMass = (1.0 / (Utils.cos(solarPosition.getZenith())
        + 0.50572 * (Math.pow((6.07995 + (90 - solarPosition.getZenith())), -1.6364))));
  }

  private void computeAoi() {
    aoiProjection = Utils.cos(ye) * Utils.cos(solarPosition.getZenith())
        + Utils.sin(ye) * Utils.sin(solarPosition.getZenith()) * Utils.cos(solarPosition.getAs() - ae);
  }
  ...
  private void computeEDirHorExtra(SolarDateTime dt) {
    ...
    double roverR0Sqrd = (1.00011 + 0.034221 * Math.cos(be) + 0.00128 * Math.sin(be) + 0.000719 * Math.cos(2 * be)
        + 7.7e-05 * Math.sin(2 * be));
  ...
  private void computeSkyClearnessIndex(double eDiffHor, double eDirHor) {
    double z = Utils.getRad(solarPosition.getZenith());
    skyClearnessIndex = ((eDiffHor + eDirHor) / eDiffHor + Utils.K * Math.pow(z, 3))
        / (1 + Utils.K * Math.pow(z, 3));
  }
